/**
 * Import 2023 Year Top Data from CSV
 *
 * Imports year top data from CSV file (MÍRALOS MORIR TOP 5 2023 para importar.csv)
 * into the database, creating YearTopParticipant, YearTopPick, and YearTopMovieStats records.
 *
 * Usage:
 *   npx tsx scripts/import-2023-year-top-csv.ts
 *   npx tsx scripts/import-2023-year-top-csv.ts --dry-run
 */
...
async function processParticipantPicks(
  ...
  // Process TOP_10 picks (PUESTO 5 to PUESTO 1, where PUESTO 1 is rank 1 and isTopPosition)
  for (let i = 0; i < topPicks.length; i++) {
    ...
    const rank = topPicks.length - i; // PUESTO 5 = rank 5, PUESTO 1 = rank 1
  ...
  // Process WORST_3 pick (PORONGA - single pick, rank 1 and isTopPosition)
  if (porongaPick && porongaPick.trim()) {
    const movieResult = await getOrCreateMovie(porongaPick, dryRun);
    if (!movieResult) {
      errors.push(`Could not create/find movie for ${porongaPick.trim()}`);
    } else {
      if (movieResult.wasCreated) {
        moviesCreated++;
      }
      ...
      if (!dryRun) {
        await prisma.yearTopPick.upsert({
          ...
          update: {
            isTopPosition: true, // Single poronga pick is always top position
          },
          create: {
            participantId,
            movieId: movieResult.movieId,
...
async function main() {
  ...
  console.log('🎬 Import 2023 Year Top Data from CSV');
  console.log('=====================================\n');
  if (dryRun) {
    console.log('🔍 DRY RUN MODE - No changes will be made\n');
  }

  const csvFilePath = path.join(process.cwd(), 'data', 'MÍRALOS MORIR TOP 5 2023 para importar.csv');
  ...
  for (const row of dataRows) {
    ...
    // Extract picks
    // Columns: (empty), PUESTO 5, PUESTO 4, PUESTO 3, PUESTO 2, PUESTO 1, (empty), PORONGA
    // So indices are: 0=name, 1=PUESTO 5, 2=PUESTO 4, 3=PUESTO 3, 4=PUESTO 2, 5=PUESTO 1, 6=empty, 7=PORONGA
    const topPicks = [row[1], row[2], row[3], row[4], row[5]].filter(Boolean); // PUESTO 5 to PUESTO 1
    const porongaPick = row[7]?.trim() || null; // PORONGA (single pick)
    ...
    if (participantId) {
      // Process picks
      const result = await processParticipantPicks(participantId, displayName, topPicks, porongaPick, dryRun);
      ...
      if (dryRun) {
        ...
        const allImdbIds = [...topPicks, porongaPick].filter(Boolean) as string[];
